...
### Changed
- Moved success / error alerts from inside the scrollable form body to the modal footer (left side) so they're always visible regardless of scroll position.
- Feedback type selector now uses a 2×2 grid instead of a stacked single column — saves vertical space, fits better next to the larger annotation canvas.
